...
import { getScript, saveScript } from '../utils/storage'

// 한국어 발표 기준 대략 분당 320자 정도로 잡아 예상 시간을 보여준다
const CHARS_PER_MINUTE = 320

export default function ScriptPracticeModal({ questionId, questionTitle, questionContent, onClose }) {
  const [script, setScript] = useState(() => (questionId ? getScript(questionId) : ''))
  ...
  return (
    ...
    >
      <div
        className="bg-surface rounded-t-2xl md:rounded-2xl shadow-xl w-full md:max-w-lg p-5 md:p-6 max-h-[92vh] overflow-y-auto"
      ...
      >
        ...
        <div className="bg-surface-alt border border-white/10 rounded-xl p-4 mb-4 text-center">
          ...
          <div className="text-4xl font-bold text-brand font-mono mb-3">{formatTime(timeLeft)}</div>
          <div className="flex gap-2">
            <button
              onClick={() => setRunning(!running)}
              disabled={timeLeft <= 0}
              className="flex-1 flex items-center justify-center gap-1.5 bg-brand hover:bg-brand-dark disabled:opacity-40 text-white font-bold py-2.5 rounded-xl transition active:scale-95"
            >
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={() => {
              if (questionId) saveScript(questionId, script)
              onClose()
            }}
